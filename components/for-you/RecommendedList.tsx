...
export default function RecommendedList() {
  ...
  useEffect(() => {
    ...
    getRecommended()
      ...
      .catch(() => {
        if (ignore) return;
        setError("Failed to load recommended");
      })
      .finally(() => {
        if (ignore) return;
  ...
  if (showSkeleton) return <RecommendedSkeleton />;
  if (error) return <div className="p-4 text-red-500">{error}</div>;

  return (
    <div className="p-4 text-black">
      <div className="mx-auto max-w-[1100px]">
        <h2 className="mb-1 text-lg font-semibold">Recommended For You</h2>
        <p className="mb-8 text-sm opacity-70">We think you’ll like these</p>
          ...
          {books.slice(0, 5).map((b) => (
            <li key={b.id} className="w-full">
              ...
              >
                ...
                <div className="mt-3 flex items-center gap-3 text-sm text-black/60">
